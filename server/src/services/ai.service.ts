...
function mergeSchemaWithContent(schema: StepSchema, aiSteps: any[]): Step[] {
  return schema.steps.map((entry, i) => {
    ...
    return {
      ...
      sceneOps: ai?.sceneOps || undefined,
    }
  })
}
...

{
  ...
  "sceneOps": {
    "highlightLines": ["AB", "A1B1"],
    "addAuxLines": [{"from": {"pointId": "A1", "position": [-1,-1,1], "label": "A₁"}, "to": {"pointId": "B1", "position": [1,-1,1], "label": "B₁"}, "dashed": true, "color": "#8b5cf6"}],
    "fadeLines": [],
    "showLabels": ["A","B","A1","B1","C","D","C1","D1"],
    "planeHighlight": {"vertices": ["A","B","B1","A1"], "color": "rgba(99,102,241,0.1)"}
  }
}
...
6. 计算步骤中写出完整算式和代入过程
7. sceneOps 字段描述该步的 3D 场景操作（用于驱动 3D 渲染）：
   - highlightLines: 需要高亮的线段 ID 数组，如 ["AB", "A1B1"]
   - addAuxLines: 需要添加的辅助线（指定起点终点 pointId、position、label）
   - fadeLines: 需要淡化的线段 ID 数组
   - showLabels: 需要显示的标签 ID 数组
   - planeHighlight: 需要高亮的面
   - addAuxPoints: 需要添加的辅助点（如中点、交点）
8. 只输出 JSON 数组，不要任何额外文字`
